...
- [TODO List](#user-content-todo-list)
  ...
  - [Снипеты](#user-content-снипеты)
    - [Включение снипетов](#user-content-включение-снипетов)
    - [Автоформатирование снипетов](#user-content-автоформатирование-снипетов)
    - [Извлечение снипетов](#user-content-извлечение-снипетов)
...
- [ ] Если не задана опция `--batch-output-root`, то генерация канонического маркдауна происходит в расположенный рядом файл (`todo.md_`).
- [ ] Если мы "экспортируем" документацию, например, в вики с использованием опции `--batch-output-root`, то мы: а) не задаём для 
      пакетной обработки документов каталоги с исходными текстами, б) явно задаём, где надо сканировать тудушки.
- [ ] Если `--batch-output-root` не задан, то это режим "inplace" генерации, тудушки процессятся, как и всё остальное, сгенерированное кладётся рядом.
- [ ] `-generate-project-todo[=file.md]`. Задаёт генерировать общий для проекта файл `todo.md]`
...
- [ ] Обработку директив `#!switch`/`#!detailing` для каждого документа делаем на предварительном этапе: так, для корневых документов будут использоваться
      переменные, заданные из командной строки, для подключаемых документов будут также аффектится переменные, устанавливаемые директивой `#!set`.
...
- [ ] Добавляем processing option - `auto-url` - и тогда, если задана переменная `__DocumentBaseUrl`, и если нет тэга URL,
