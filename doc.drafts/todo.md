...
- **batch-режим**
  ...
  - [ ] Пофиксить генерируемый файл git-add.bat - сделать вывод в UTF-8 и добавить включение UTF-8 в батнике (см. папку git-add)
...
- **Баги**
  ...
  - [ ] Баг с генерацией оглавления. 1) Оглавление почему-то генерируется в список с отступом. 2) На некоторых документах отступ получается слишком большим,
        и перестаёт восприниматься как список (и ссылки в нем перестают парсится как ссылки, по крайней мере в GitLab).
  - [ ] Баг с несколькими авторами. Пробовал, как в YAML: список с минусом спереди - вообще пропало, пробовал в одну строчку через запятую - второе имя почему-то в кавычках.
