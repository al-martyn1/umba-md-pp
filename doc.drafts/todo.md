...
**URL**: https://github.com/al-martyn1/umba-md-pp/blob/main//doc.drafts/todo.md_
...
- [TODO List](#user-content-todo-list)
  ...
  - [Директивы UMBA-MD-PP - #!meta - вставка метатегов в документ](#user-content-директивы-umba-md-pp---meta---вставка-метатегов-в-документ)
  - [Метатеги](#user-content-метатеги)
  ...
  - [Извлечение фрагментов кода](#user-content-извлечение-фрагментов-кода)
...
- [ ] `gvp-meta`. `Graphviz` - коментарий сишный, `//`, метаинфа в `Graphviz` начинается с `//---` в самом начале файла.
- [ ] `gvp-meta`. `PlantUML` - коментарий - одинарная кавычка, `'`, метаинфа в `Graphviz` начинается с `'---` в самом начале файла.
- [ ] `gvp-meta`. Метаинфа `Graphviz`/`PlantUML` заканчивается точно так же, как начинается.
- [ ] `gvp-meta`. Пробелы в концах строк игнорируются.
- [ ] `gvp-meta`. Метаинфа в `Graphviz`/`PlantUML` файлах внутри begin/end мета-сепараторов должна начинаться с одного префикса.
- [ ] `gvp-meta`. Метаинфа в `Graphviz`/`PlantUML` после удаления специфики `Graphviz`/`PlantUML` является метаинфой в формате  YAML.
...
## Директивы UMBA-MD-PP - #!meta - вставка метатегов в документ
...
- [ ] `#!meta`. Нужно добавить фичу макроподстановок в метатегах. Тогда мы задаём в командной строке `DocumentBaseURL`/`BaseURL` и можем его 
      использовать в метатеге "URL".


## Метатеги

- [ ] При пакетной обработке задаем переменную `__DocumentRelFileName` - путь к документу относительно каталога поиска,
      в котором он найден. Расширение убирается, если стоит опция обрезки расширений.
- [ ] Добавляем processing option - `auto-url` - и тогда, если задана переменная `__DocumentBaseUrl`, то если нет тэга URL,
...
- [ ] В какой момент проверять наличие тэга URL и при отсутствии генерить автоматически - пока не понял.
