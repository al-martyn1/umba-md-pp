...
- [ ] `--notes-single-output-title=TYPE:TITLE` - задаёт текст 'заголовка' для заметок, когда они помещаются в один файл.

- [ ] `--notes-single-output-title-format=[$(TITLE)]` - формат 'заголовка' заметок, когда они помещаются в один файл.

- [ ] `--notes-source-info-format='$(FILE):$(LINE)'` - format информации о расположении заметки в исходных текстах.
