...
- [ ] Сохранение коллекции тудушек и остального - либо в явно заданный каталог, и тогда там создаются файлы
  ...
  в каждом каталоге с сорцами. Если задаём `--todo-output-path=PATH` - то сохраняем все файлы в данном каталоге.

- [ ] `--todo-filename=TYPE:NAME` - задает имя файла для заметок типа `TYPE`, без расширения. `--notes-filename` - алиас,
  и, наверное, даже основное имя.

- [ ] `--todo-ext=EXT` - задаем расширение для всех файлов заметок. `--notes-ext=EXT` - алиас.

- [ ] `--todo-marker=TYPE:NAME` - стартовые маркеры: `TODO`, `ATT`, `ATTE`, `ATTENTION`, `NOTE`, `UNDONE`, `!!!`. Последнее
...
- [ ] `--todo-strip==TYPE:TEXT` - обрезаем текст заметки спереди. Сюда будут добавлены: `:`, `!!!`, `-`.
