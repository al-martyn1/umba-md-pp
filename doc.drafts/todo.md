...
- **Директивы UMBA-MD-PP**
  ...
  - [ ] `#!set{}`. Директива `set` работает только на текущий файл. Это про batch-режим. Перед обработкой какждого файла в batch-режиме надо делать
  ...
  - [ ] `#!meta`. Цель - вставлять в документ данные из метатегов. Пример использования: GitLab отображает метатеги при просмотре документа в вики,
        ...
        Можно включить печать колонтитулов, там адрес печатается, но колонтитул печатается вплотную к краям листа, и при "слепой" зоне принтера в пару мм
        колонтитулы уже не прочитать. Поэтому решено добавить возможность вставлять метатеги в документ, отбивая их от тела документа горизонтальным
  ...
  - [ ] `Метатеги`. Нужно добавить фичу макроподстановок в метатегах. Тогда мы задаём `DocumentBaseURL`/`BaseURL` и можем его использовать в метатеге "URL".
  - [ ] `sidebar.
