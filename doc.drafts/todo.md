...
- [ ] Заметки разных типов могут помещаться в один файл. `--notes-single-output=todo` - все заметки помещаем в файл `todo` (-S?).

- [ ] `--notes-single-output-title=TYPE:NAME` - задаёт текст 'заголовка' для заметок, когда они помещаются в один файл.

- [ ] `--notes-single-output-title-decoration=[$(TITLE)]` - 'декорация' 'заголовка' заметок, когда они помещаются в один файл.

- [ ] `--notes-source-info-decoration='$(FILE):$(LINE)'` - 'декорация' информации о расположении заметки в исходных текстах.

- [ ] Выходной тип используется такой же, как и для списка описаний.
...
- [ ] Но, если у нас одноострочный коммент, то в следующем однострочном комменте может быть новая заметка.

- [ ] Храним как список заметок. Заметка содержит тип, текст, наличие и значение маркера, имя исходного файла и строка.

- [ ] Сохраняем эти списки заметок в мапах, где ключем идёт имя каталога, в котором расположен файл с заметкой.

- [ ] При формировании выходных файлов, если генеация идёт в один общий файл в `output` каталоге, имена папок используются, как заголовки.
