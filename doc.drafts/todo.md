...
- **Директивы UMBA-MD-PP**
  ...
  - [ ] `#!meta`. `--document-meta-tags=author,url` - задаёт список метаданных, которые будут вставлены в документ по запросу
        (опцией командной строки или директивой `#!meta` без параметров).
  - [ ] `#!meta`. `--document-add-meta-tags[=true]` - явно задаёт вставить метатеги в тело документа (а не в метатеги).
