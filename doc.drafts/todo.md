...
- **Командная строка и .md-pp.options файлы опций**
  ...
  - [ ] `.md-pp.options` и `umba-md-pp.options` - обрабатываем только один из них, предпочтение для `.md-pp.options`. Или оба?
...
- **Директивы UMBA-MD-PP**
  ...
  - [ ] `detailing`. По директиве `switch` напрашивается вариант использования - уровень детализации. Детализация подразумевает,
        ...
        Допустим, у нас 
        Ссылки про уровень детализации: https://en.wikipedia.org/wiki/Level_of_detail, https://en.wikipedia.org/wiki/Level_of_detail_(writing)
