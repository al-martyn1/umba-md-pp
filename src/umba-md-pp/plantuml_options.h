...
struct PlantUmlOptions
{
    ...
    std::string              saveFilename = "diagram";
    ...
    //LOG_INFO("plantuml") << "PlantUML generator configuration"
    void logConfig() const
    {
        LOG_INFO("plantuml") << "TargetFormat : " << enum_serialize(targetFormat) << "\n";
        LOG_INFO("plantuml") << "DiagramType  : " << enum_serialize(diagramType) << "\n";
        LOG_INFO("plantuml") << "Scale (%)    : " << scale << "\n";
        LOG_INFO("plantuml") << "Filename Autogeneration: " << (useFilenameAutogeneration?"true":"false") << "\n";
        LOG_INFO("plantuml") << "A/g index    : " << autoGeneratedIndex << "\n";
        LOG_INFO("plantuml") << "A/g tmp index: " << autoGeneratedTempDotIndex << "\n";
        LOG_INFO("plantuml") << "Save Filename: " << saveFilename << "\n";
        LOG_INFO("plantuml") << "Save Path    : " << savePath << "\n";
        LOG_INFO("plantuml") << "Show Labels  : " << (showLabels?"true":"false") << "\n";
    }
    ...
    bool deleteHashFile() const
    {
        ...
        {
            LOG_INFO("plantuml") << "Delete hash file: '" << hashFileName << "'\n";
            if (!umba::filesys::deleteFile(hashFileName))
            {
                res = false;
                LOG_WARN("plantuml") << "Failed to delete hash file: '" << hashFileName << "': " << umba::shellapi::getErrorMessage(umba::shellapi::getLastError()) << "\n";
            }
        }
    ...
    std::string generateOutputFilename(bool flattenImageLinks, std::string *pGeneratedIndex ) const
    {
        std::string name;

        if (useFilenameAutogeneration)
        {
            auto idxStr = generateFileIndexString(autoGeneratedIndex++, 4);
            name = saveFilename + "_" + idxStr;
            if (pGeneratedIndex)
               *pGeneratedIndex = idxStr;
        }
        else
        {
            name = saveFilename;
                                ...
                                ) const
    {
        ...
        if (java.empty())
        {
            LOG_WARN("plantuml") << "Failed to generate PlantUML command line: JAVA not found\n";
            return false;
        }

        if (plantUmlJar.empty())
        {
            LOG_WARN("plantuml") << "Failed to generate PlantUML command line: PlantUML JAR not found\n";
            return false;
        }
