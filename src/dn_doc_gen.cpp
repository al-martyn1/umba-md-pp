...
int main(int argc, char* argv[])
{
    ...
    #if 0
    auto testIsFlagsEnum = [](auto v, std::string str)
    {
        std::cout << "enum_is_flags(" << str << "): " << (enum_is_flags(v) ? "true" : "false") << "\n";
    };

    #define TEST_IS_FLAGS_ENUM(e) \
        testIsFlagsEnum(e, #e)

    TEST_IS_FLAGS_ENUM(marty_draw_context::Endianness::invalid);
    TEST_IS_FLAGS_ENUM(marty_draw_context::FontStyleFlags::invalid);
    //TEST_IS_FLAGS_ENUM(marty_draw_context::);
    #endif
    ...
    // std::string                                           batFileLineTemplate;
        ...
        // addMdCommonStr("enumeration-index-text"              , "$(Name) enumeration");
        // addMdCommonStr("flags-index-text"                    , "$(Name) flags");
        // addMdCommonStr("namespace-index-text"                , "$(Name) namespace");
        // addMdCommonStr("class-index-text"                    , "$(Name) class");
    ...
    marty_simplesquirrel::EnumScriptGenerationOptions 
      generationOptions = { marty_simplesquirrel::EnumScriptGenerationType::trTemplate // mdDoc // trTemplate // multiLineScript
                          , 3         // mdSectionLevel
                          , true      // mdAddVals
                          , "en"      // mdLang
                          , "-------" // mdSectionSep
    ...
    // umba::toUtf8(str);
    auto dcEnumsSquirrelScript = marty_vk::simplesquirrel::enumsExposeMakeScript("Vk", generationOptions)
                               + _SC("\n\n\n")
                               + marty_draw_context::simplesquirrel::enumsExposeMakeScript( generationOptions // singleLineScript
                                                                                          ,  /* const std::string &prefix = */  "Drawing"
                                                                                          )
                               + _SC("\n\n\n")
                               + dotNut::simplesquirrel::dotNutCoreEnumsExposeMakeScript(true /* createTable */ , generationOptions)
                               ;


    std::cout << umba::toUtf8(dcEnumsSquirrelScript) << "\n\n\n";
#endif
