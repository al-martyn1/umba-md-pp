...
**--info=info-type1[,+info-type2,-info-type]** - Make info messages enabled/disabled, '+' (or nothing) - enable message, '-' - disable it. Type is one of: `config`, `graphviz`, `opt-files`, `plantuml`, `snippet-lookup`, `strip-extentions`.
