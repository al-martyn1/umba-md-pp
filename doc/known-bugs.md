  ...
  - [ ] Пакетный режим MDPP: сканируем каталог(и) (рекурсивно или нет), обрабатываем все подходящие под маску файлы. При рекурсивном сканировании исключаем
        каталоги, подходящие под маску (чтобы не обрабатывать каталоги вида `_md`/`md_`, в которых лежат части документов, не являющиеся самостоятельными 
        документами, и предназначены только для включения).
        При этом у каждого файла есть относительный путь до корневого каталога сканирования, и он будет применяться при записи файла в выходной каталог,
        если тот задан. Если выходной каталог не задан, результирующий файл будет записан в исходный каталог.
  - [ ] Сканирование производится по расширениям, заданным для MDPP: `_md`, `md_`, `_markdown`, `markdown_`. Имена выходных файлов формируются заменой
        расширения при помощи удаления начальных и конечных подчёркиваний.
