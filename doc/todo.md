# Известные проблемы и TODO
  ...
  - [Известные проблемы и TODO](#user-content-известные-проблемы-и-todo)
...
## Важные задачи в библиотеках
 ...
 - [ ] Генерация MD по параметрам командной строки. Также нужно скрестить это с подсистемой переводов TR. 
   Нужно генерить и рыбу переводов (в формате JSON/XML). В командной строке нужно добавить опции --help-lang, --help-translation-format.
...
## TODO List
  ...
  - **Макроподстановки**
    ...
    - [ ] Macro names register matter - регистр символов в имени макросов имеет значение. По умолчанию, без включения опций 
  ...
  - **Таблицы**
    ...
    - [ ] Упрощённые таблицы из CSV - используем тэг <csv> и </csv> - строка начинается данного тэга. 
          После стартового тэга может идти строка таблицы с выравниванием.
    - [ ] В матстатистику по автоопределению разделителя не играем - лень. По умолчанию разделитель - ',' (запятая). 
          Разделитель можно указать после открывающего тэга, перед строкой выравнивания.
          Можно использовать ';'. Можно использовать 't'/'T' - символ табуляции.
  ...
  - **Вьювер**
    ...
    - [ ] Если опция копирования задана, то открываем документ уже в исходной локации, чтобы можно было сразу подредактировать его и сохранить.
