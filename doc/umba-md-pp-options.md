...
**--verbose[=MODE]** - Allow overwrite existing file.
Flag option, allowed values: `+`/`1`/`Y(es)` or /`0`/`N(o)`/`False`, default value: TRUE.
...
**--set-meta-tag-type=TYPE:TAG[,TAG...]** - Set meta tag type.

**--serialize-meta-tags=TAG[,TAG...]** - Set/append meta tag serialize list.
...
**--graphviz-output-format=DPI** - Set Graphviz tools output format (SVG/PNG/BMP).
